plot_umap_hulls <- function(adata_t, cluster, color_code, scatter = FALSE) {
  plot_data <-adata_t$obs %>% left_join(
  ...
  ) %>% rename(cluster = !!sym(cluster),
               color_code = !!sym(color_code)) %>% mutate(cluster = str_to_sentence(cluster))
  pal <- plot_data %>%
  ...
  if (scatter){
    adata_t$uns$UMAP_hulls$hulls %>%
      ...
      geom_point(data = plot_data,
                 ...
                 inherit.aes = F,
                 size = 0.5,
                 show.legend = F) + scale_color_manual(values = pal) +
      coord_fixed()  +
      labs(x = "UMAP1", y = "UMAP2")

  } else {
    adata_t$uns$UMAP_hulls$hulls %>% rename(UMAP1 = X, UMAP2 = Y) %>%
      ggplot(aes(UMAP1,UMAP2, group = paste(cluster, sub_cluster, landmass), fill = as.factor(cluster))) +
      geom_polygon(show.legend = F,
                   color = "black") +
      theme_classic() +
      scale_fill_manual(values = pal) +
      geom_point(data = adata_t$uns$UMAP_hulls$center_density,
                 aes(x, y),
                 inherit.aes = F,
                 size = 0.2,
                 show.legend = F) +
