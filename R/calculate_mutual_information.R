...
MI_scores <- adata_t$uns$consensus_clustering %>% select(gene, cluster) %>%
  calculate_MI(., MI_GOterms, nrep = 500)

write_tsv(MI_scores, savefile_MIGO)

gene_distances <- adata_t$obs %>% left_join(
...
) %>% select(gene =ensembl_id, cluster, UMAP_1_scaled = UMAP1,UMAP_2_scaled = UMAP2) %>% group_by(cluster) %>%  do({
  dat <- .
  dat %>%
