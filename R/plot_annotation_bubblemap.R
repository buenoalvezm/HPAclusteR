...
plot_data_all <-
  all_res %>%
  group_by_all() %>%
  mutate(capped_odds_ratio = min(c(
    odds_ratio,
    50
  ))) %>%
  ungroup()
...
plot_legend_settings <-
  tibble(
    odds_ratio_str = c(1, 10, 20, 30, 40, ">50"),
    y = 1:6,
...
plots <-
  pbapply::pblapply(
    1:nrow(bubble_heatmap_settings),
    function(i) {
      i <<- i
      ...
      plot_order <-
        list(
          cluster = unique(c(
            with(
              plot_clustering$cluster,
              labels[order]
            ),
            plot_levels
          )),
          ID = with(
            plot_clustering$tissue,
            labels[order]
          )
        )
      ...
      plot <-
        ...
        geom_point(
          data = expand_grid(
            cluster = plot_order$cluster,
            ID = plot_order$ID
          ) %>%
            mutate(
              cluster = factor(cluster, unique(cluster)),
