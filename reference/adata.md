...
- X:
  ...
  A data frame with 20,162 rows and 79 columns representing the main
  data matrix (e.g., gene expression values).

- layers:

  A list containing additional data matrices, such as normalized counts
  (`ntpm`).

- obs:

  A data frame with 78 rows and 7 columns containing metadata for
  observations (e.g., tissues).
...
- var_names_col:
  ...
  A string specifying the column used for variable names (`ensembl_id`).
...
#> <AnnDatR>
#>   Public:
#>     X: spec_tbl_df, tbl_df, tbl, data.frame
...
#>     obs_names: function () 
#>     obs_names_col: tissue_id
#>     obsm: NULL
#>     raw: NULL
#>     uns: NULL
#>     var: spec_tbl_df, tbl_df, tbl, data.frame
#>     var_names: function () 
#>     var_names_col: ensembl_id
```
